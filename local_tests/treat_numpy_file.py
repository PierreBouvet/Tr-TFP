...
# Extract results
source_filepath = '/Volumes/LAUDATE/Data/260100 - TR-TFP/Water - 5ms pulse 10V - bipolar - channels 172 to 322 - 532nm-3mm-150nm.npy'
...
# # Get observed window
# # f_min = -6188843526.702797
# # f_max = -3479039157.6662626
...
# # plt.vlines(freq[pos_min], 0, np.max(results[-1, :1024]), colors='r')
# # plt.vlines(freq[pos_max], 0, np.max(results[-1, :1024]), colors='r')
# # plt.vlines(freq[pos_min-dt_ms], 0, np.max(results[-1, :1024]), colors='g')
# # plt.vlines(freq[pos_max+dt_ms], 0, np.max(results[-1, :1024]), colors='g')
...
# treat.add_point(position_center_window=-7, type_pnt="Stokes", window_width=2)
